  ...
  useTranslation: () => ({
    t: (key, options = {}) => {
      const translations = {
        ...
        "comparison.actions.clearDescription": `Esta ação removerá todos os ${
          options.count || 0
        } Pokémon da comparação. Esta ação não pode ser desfeita.`,
        ...
        "comparison.actions.maxReached":
          "Máximo de 4 Pokémon podem ser comparados simultaneamente",
...
describe("Comparison Integration Tests", () => {
  ...
  describe("Empty Comparison State", () => {
    it("should display empty state when no pokémon in comparison", () => {
      ...
      expect(
        screen.getByText("🔍 Nenhum Pokémon para comparar"),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          "Adicione Pokémon à comparação clicando no ícone de comparação nas páginas de detalhes.",
  ...
  describe("Comparison with Data", () => {
    ...
    it("should open clear confirmation dialog when clear all is clicked", () => {
      ...
      expect(
        screen.getByText(
          "Esta ação removerá todos os 2 Pokémon da comparação. Esta ação não pode ser desfeita.",
        ),
      ).toBeInTheDocument();
    });
  ...
  describe("Adding Pokémon to Comparison", () => {
    ...
    it("should show error when trying to add more than maximum allowed", async () => {
      ...
      await waitFor(() => {
        const state = store.getState();
        expect(state.comparison.error).toBe(
          "Máximo de 4 Pokémon podem ser comparados simultaneamente",
        );
      });
    });
  ...
  describe("Statistics Calculation", () => {
    ...
    it("should identify highest and lowest stats", () => {
      ...
      expect(
        screen.getByText(/Maior Stat.*Ataque Especial.*65.*bulbasaur/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Menor Stat.*HP.*39.*charmander/),
      ).toBeInTheDocument();
    });
