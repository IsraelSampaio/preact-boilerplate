  ...
  useTranslation: () => ({
    t: (key, options = {}) => {
      const translations = {
        ...
        'favorites.subtitle': `Você tem ${options.count || 0} Pokémon favoritos.`,
        'favorites.subtitle_zero': 'Você ainda não tem nenhum Pokémon favorito.',
        'favorites.empty.title': '🤔 Nenhum favorito ainda',
        'favorites.empty.description': 'Explore a Pokédex e adicione seus Pokémon favoritos clicando no ícone de coração!',
        ...
        'favorites.actions.clearDescription': `Esta ação removerá todos os ${options.count || 0} Pokémon dos seus favoritos. Esta ação não pode ser desfeita.`,
        'favorites.actions.clearButton': 'Sim, limpar todos',
        'favorites.search.placeholder': 'Buscar nos favoritos...',
        'favorites.search.noResults': 'Nenhum favorito encontrado com os filtros aplicados.',
...
describe('Favorites Integration Tests', () => {
  ...
  describe('Empty Favorites State', () => {
    it('should display empty state when no favorites exist', () => {
      ...
      expect(screen.getByText('Você ainda não tem nenhum Pokémon favorito.')).toBeInTheDocument();
    ...
    it('should redirect to pokemon list when clicking explore button', () => {
      ...
      expect(exploreButton.closest('button')).toHaveAttribute('href', '/pokemon');
    });
  });
  ...
  describe('Favorites with Data', () => {
    it('should display favorites list when data exists', () => {
      ...
      expect(screen.getByText('Você tem 2 Pokémon favoritos.')).toBeInTheDocument();
    ...
    it('should open clear confirmation dialog', () => {
      ...
      expect(screen.getByText('Limpar todos os favoritos?')).toBeInTheDocument();
      expect(screen.getByText('Esta ação removerá todos os 2 Pokémon dos seus favoritos. Esta ação não pode ser desfeita.')).toBeInTheDocument();
    });
    ...
    it('should cancel clear action when user cancels', () => {
      ...
      // Dialog deve fechar
      expect(screen.queryByText('Limpar todos os favoritos?')).not.toBeInTheDocument();
  ...
  describe('Local Storage Integration', () => {
    it('should load favorites from localStorage on initialization', () => {
      ...
      expect(mockLocalStorage.getItem).toHaveBeenCalledWith('pokemon-app-favorites');
    });
