...
i18n
  ...
  .init({
    ...
    detection: {
      ...
      convertDetectedLanguage: (lng) => {
        ...
        if (["pt-BR", "en"].includes(lng)) {
          return lng;
        }

        // Converter códigos comuns do navegador para nossos códigos
        if (lng.startsWith("pt-") || lng === "pt") {
          return "pt-BR";
        }
        if (lng.startsWith("en-") || lng === "en") {
          return "en";
        }
  ...
  .then(() => {
    ...
    if (!supportedLngs.includes(currentLang)) {
      console.warn(
        `Current language ${currentLang} is not in supported languages. Falling back to pt-BR.`,
      );
